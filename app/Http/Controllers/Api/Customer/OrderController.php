    ...
    public function create(CustomerAddRequest $request)
    {
        ...
        if ($customer->hasRole('customer')) {
            ...
            // $aa = $data['data'][0];
                ...
                // dd($customer->restaurant->admin_id);
                // dd($request->delivery_price);
                $invoice = Invoice::create([
                    'status' => InvoiceStatus::WAITING->value,
                    'restaurant_id' => $customer->restaurant_id,
                    'admin_id' => $customer->restaurant->admin_id,
                ...
                try {
                    $this->firebaseService->sendNotification(
                        $invoice->admin->fcm_token,
                        $title,
                        $body,
                        $data2
                    );
                } catch (\Exception $e) {
                    // Check if the error indicates invalid/expired token
                    if (str_contains($e->getMessage(), 'Requested entity was not found')) {
                        // Clean up invalid token
                        $invoice->admin->update(['fcm_token' => null]);
                    }
